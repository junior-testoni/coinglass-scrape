...
def fetch(endpoint: str, params: dict | None = None, api_key: str | None = None) -> dict:
    ...
    if api_key:
        # According to the API documentation the key must be sent via the
        # ``CG-API-KEY`` header. Using an incorrect header will result in the
        # server reporting a missing API key.
